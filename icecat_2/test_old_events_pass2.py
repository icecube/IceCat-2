...
for i in range(len(run)):
    try:
        ...
        outfile='run'+str(run[i])+'_eventid'+str(eventid[i])+'_old_pass2.i3'
        i3file_retriever.retrieve_i3file_pass2(int(run[i]),int(eventid[i]),outfile)
        ## 2. Add OnlineL2_SplineMPE_TruncatedEnergy to the i3file
        add_truncated_energy_i3files.add_truncated_energy_i3file(run[i],eventid[i],'old_pass2')
        ...
        infile='run'+str(run[i])+'_eventid'+str(eventid[i])+'_old_pass2_te.i3'

...
for i in range(10,len(run)):
    try:
        ## 1. Retrieve i3file for each event in the table from i3Live
        outfile='run'+str(run[i])+'_eventid'+str(eventid[i])+'.i3'
        i3file_retriever.retrieve_i3file(run[i],eventid[i],outfile)
        ## 2. Add OnlineL2_SplineMPE_TruncatedEnergy to the i3file
        get_truncated_energy_orig.add_truncated_energy_orig_i3file(run[i],eventid[i])
